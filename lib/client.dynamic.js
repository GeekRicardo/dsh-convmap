...
`
    ...
    function countUserNodes(chat) {
      if (!chat || !chat.nodes || typeof chat.nodes.get !== 'function' || !Array.isArray(chat.order)) return 0
      let count = 0
      for (const key of chat.order) {
        const node = chat.nodes.get(key)
        if (node && node.kind === 'user') count++
      }
      return count
    }
    ...
    function ConversationMap(props) {
      const userNodeCount = props.useSession((snapshot) => countUserNodes(snapshot.chat))
