...
//    的文本是该轮回复摘要。
//    整轮都没有人发言时（插件注入的通知起的轮次）退而用该轮第一条注入消息，
//    否则「代理自己跑起来的」会话整条地图会是空的。
...
//
// 摘要在 host 侧就截断（提问 200 字 / 回复 800 字）：预览卡可以滚动，所以要给
// 得出足够滚的量，但整段正文仍然没必要过网络——这一刀在长会话下省掉的是数量级
// 的传输量。「最多能预览多少」就由下面这两个常量定死。
...
/** 提问摘要上限（字符数，按 code point 计）；预览卡标题最多展示这么多。 */
const PROMPT_LIMIT = 200;

/** 回复摘要上限（字符数，按 code point 计）；预览卡正文滚到底就是这么多。 */
 ...
 * 得到一条永远跳不过去的刻度。
 * @param {{ entries: object[], turn: number }} state - 折叠状态（候选轮次 + 当前轮号）。
...
function foldEvent(state, event) {
  ...
  if (event.type === "user/message") {
    ...
    state.entries.push({
      ...
      seq: event.seq,
      turn: state.turn,
      human: data.source.kind === "user",
    });
  } else if (event.type === "assistant/message" && state.entries.length > 0) {
    const message = event.data && event.data.message;
    const text = textOfContent(message && message.content);
    if (text.trim()) state.entries[state.entries.length - 1].response = snippet(text, RESPONSE_LIMIT);
  }
}

/**
 * 收敛候选：一轮里只要有人发的消息，就只保留人的；整轮都没有人发言
 * （插件注入的通知起的轮次，例如审批变更、Cordis run 完成），就留这一轮的
 * 第一条注入消息当刻度——否则这种「代理自己跑起来的」会话整条地图是空的。
 * @param {object[]} entries - 折出的候选轮次。
 * @returns {{ key: string, prompt: string, response: string, seq: number }[]} 最终轮次列表。
 */
function selectTurns(entries) {
  const humanTurns = new Set();
  for (const entry of entries) if (entry.human) humanTurns.add(entry.turn);
  const kept = [];
  const keptOfTurn = new Map();
  for (const entry of entries) {
    if (entry.human || (!humanTurns.has(entry.turn) && !keptOfTurn.has(entry.turn))) {
      kept.push(entry);
      keptOfTurn.set(entry.turn, entry);
      continue;
    }
    // 被丢掉的候选身上挂着的回复要交给同一轮留下的那条刻度，否则本轮的
    // 回复摘要会跟着候选一起消失。
    const owner = keptOfTurn.get(entry.turn);
    if (owner !== undefined && entry.response) owner.response = entry.response;
  }
  return kept.map(({ key, prompt, response, seq }) => ({ key, prompt, response, seq }));
}
...
function buildTurns(events) {
  const state = { entries: [], turn: 0 };
  for (const event of Array.isArray(events) ? events : []) foldEvent(state, event);
  return selectTurns(state.entries);
}
...
function buildTurnsFromLog(content) {
  ...
  for (const line of String(content || "").split("\n")) {
    if (
      line.indexOf('"user/message"') < 0
      && line.indexOf('"assistant/message"') < 0
      && line.indexOf('"turn/start"') < 0
    ) continue;
    let event;
    try {
...
export { apply, inject, buildTurns, buildTurnsFromLog, chatAnchorKey, selectTurns, snippet, textOfContent };
