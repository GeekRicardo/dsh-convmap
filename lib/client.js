...
//       预览卡的去留按位置判定：指针在刻度区、卡片、或两者之间那条过道里就
//       留着（够把鼠标挪进卡），去了别处立刻收；卡本身可滚（正文最多就是
//       host 截给的那些字），滚动不外溢到对话；
...
`
    ...
    function ConversationMap(props) {
      ...
      geoRef.current = geo
      const previewAnchorRef = React.useRef(0)
      previewAnchorRef.current = previewAnchor
      ...
      const inSafeZone = (x, y) => {
        ...
        if (x >= railRect.left && x <= railRect.right && y >= railRect.top && y <= railRect.bottom) return true
        const card = previewRef.current
        if (card === null) return false
        const cardRect = card.getBoundingClientRect()
        if (x >= cardRect.left && x <= cardRect.right && y >= cardRect.top && y <= cardRect.bottom) return true
        const anchor = previewAnchorRef.current
        return x >= railRect.right && x <= cardRect.left
          && y >= Math.min(cardRect.top, anchor - PITCH)
          && y <= Math.max(cardRect.bottom, anchor + PITCH)
      }

      React.useEffect(() => {
        if (emphasized === null) return undefined
        const onMove = (ev) => {
          if (!inSafeZone(ev.clientX, ev.clientY)) setEmphasized(null)
        }
        const onLeaveWindow = () => setEmphasized(null)
